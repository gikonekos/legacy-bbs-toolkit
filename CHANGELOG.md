...
### 追加
- 非エンジニア向けQUICKSTART.md追加。インストール〜基本的な使い方・結果の見方・
  トラブルシューティングを平易な日本語でまとめた。
- CLI出力を全面的に見直し、既定表示を日本語の分かりやすいサマリーに変更
  （✅/⚠️による一目での状態表示、パーサーnamespaceの日本語系統名表示、
  警告・メールアドレス検出・年情報なし投稿数などを平文で説明）。
  従来の技術的な詳細情報（namespace文字列・selection内部情報等）は
  `--verbose` オプションで表示する形に分離。
